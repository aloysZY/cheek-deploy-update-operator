...
func (r *CheekDeployUpdateReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// 检查自定义 cr 是否存在,这其实是其他调用这个 CR 的时候设置的
	...
	if err := r.Get(ctx, req.NamespacedName, &cheekDeployUpdate); err != nil {
		log.Error(err, "unable to fetch cheekDeployUpdate")
	...
	/*	isFinished := func(deployment *appsv1.Deployment) corev1.ConditionStatus {
		for _, c := range deployment.Status.Conditions {
			// NewReplicaSetAvailable 常量没找到在那定义的
